...
INTERPRETATION_PROMPT = (
    "You are reading a visual composition and finding the scene hidden inside it.\n\n"
    "Step 1 — Composition map (visual facts only):\n"
    "Describe each object as if you have never seen it before and do not know what it is.\n"
    "No functions. No names. Only shape.\n"
    "For each object state:\n"
    "  • silhouette in geometric terms (tall cylinder / flat rectangle / curved wedge / L-shape / etc.)\n"
    "  • exact position in the frame (top-left / lower-center / right-edge / etc.)\n"
    "  • size relative to other objects (largest / smallest / half the size of object 2 / etc.)\n"
    "  • orientation (upright / tilted left / horizontal / leaning against another / etc.)\n"
    "Then describe the skeleton: connect every object center — what overall shape does the group make?\n"
    "Note clusters, alignments, and empty regions.\n\n"
    "Step 2 — Constrained interpretation:\n"
    "Generate three candidate interpretations — each based ONLY on the shapes and positions from Step 1,\n"
    "not on what the objects are or what they do.\n"
    "For each candidate:\n"
    "  (a) cite which specific shapes/positions/sizes support it\n"
    "  (b) check: does any shape or position from Step 1 contradict it?\n"
    "Discard any candidate contradicted by a visual fact.\n"
    "Discard the most obvious reading.\n"
    "Choose the interpretation that is most surprising yet supported by the most visual facts.\n"
    "It must make someone say: 'I can't unsee it now' — because it was always there in the shapes.\n\n"
    "Step 3 — Object roles:\n"
    "For each object, name the specific visual feature (exact edge, contour, silhouette, proportion)\n"
    "that IS the scene element — not 'it looks like', but 'its [feature] IS [element]'.\n\n"
    ...
    "Do not redraw the object — only what grows outward from the named feature.\n\n"
    "OUTPUT FORMAT (strict — follow exactly):\n\n"
    "INTERPRETATION: <single title, max 10 words>\n\n"
    "OBJECT 1: <shape/silhouette> | POSITION: <exact location> | FEATURE: <specific edge or contour> | ROLE: <what that shape becomes>\n"
    "OBJECT 2: <shape/silhouette> | POSITION: <exact location> | FEATURE: <specific edge or contour> | ROLE: <what that shape becomes>\n"
    "... (one line per object)\n\n"
    "VISUAL EXPANSION:\n"
