...
INTERPRETATION_PROMPT = (
    ...
    "Step 1 — Composition map:\n"
    "Encode the image as a precise set of visual facts. For each object state:\n"
    "  • silhouette / shape (in geometric terms — cylinder, flat rectangle, curved wedge, etc.)\n"
    "  • exact position in the frame (top-left / lower-center / right-edge / etc.)\n"
    "  • size relative to other objects (largest / smallest / similar)\n"
    "  • orientation (upright / tilted / horizontal / leaning)\n"
    "Then describe the skeleton of the arrangement: if you connected every object center with lines,\n"
    "what overall shape does the group make? Where are the clusters? Where is empty space?\n\n"
    "Step 2 — Constrained interpretation:\n"
    "Generate three candidate interpretations. For each one you MUST:\n"
    "  (a) list which specific visual facts from Step 1 support it\n"
    "  (b) check: does any visual fact from Step 1 contradict it?\n"
    "Discard any candidate contradicted by a visual fact.\n"
    "Discard the most obvious reading.\n"
    "Choose the interpretation supported by the most facts that still surprises you.\n"
    "It must make someone say: 'I can't unsee it now' — because it was always there.\n\n"
    "Step 3 — Object roles:\n"
    "For each object, state which specific visual feature (exact edge, contour, silhouette, proportion)\n"
    "IS the scene element. Not 'it looks like' — but 'its [specific feature] IS [element]'.\n\n"
    "Step 4 — Drawing from the feature:\n"
    "For each object, describe the single line that extends FROM the named feature and completes its role.\n"
    "Do not redraw the object. Only what grows outward from the named feature.\n\n"
    "OUTPUT FORMAT (strict — follow exactly):\n\n"
    "INTERPRETATION: <single title, max 10 words>\n\n"
    "OBJECT 1: <shape/silhouette> | POSITION: <exact location> | FEATURE: <specific edge or contour> | ROLE: <what that feature IS>\n"
    "OBJECT 2: <shape/silhouette> | POSITION: <exact location> | FEATURE: <specific edge or contour> | ROLE: <what that feature IS>\n"
    "... (one line per object)\n\n"
    "VISUAL EXPANSION:\n"
