...
INTERPRETATION_PROMPT = (
    "Analyze this image.\n\n"
    "Step 1 — List every distinct physical object you see and where it sits.\n\n"
    "Step 2 — For EACH object, write its METAMORPHOSIS: what it becomes in a fictional world.\n"
    "Use this exact format for each:\n"
    "  [real object] → [what it has become]\n"
    "Examples of the transformation quality required:\n"
    "  water bottle → a lighthouse on a flooded plain\n"
    "  laptop → a cliff face covered in ancient carvings\n"
    "  coffee cup → the mouth of a dormant volcano\n"
    "  notebook → a frozen lake seen from above\n"
    "  pen → a javelin thrown by a giant and left quivering in the earth\n"
    "The transformation must use the object's real shape, color, and size as its raw material.\n"
    "It must NOT be a metaphor or description — it must be a concrete thing from another world.\n\n"
    "Step 3 — Invent ONE scene that connects ALL the metamorphosed objects into a single world.\n"
    "• Write the scene using ONLY the transformed identities — NEVER mention the real objects.\n"
    "• The scene sentence must read like a line from a myth or a story, not a room description.\n"
    "• It must be specific to THIS exact arrangement — swap or move one thing and it collapses.\n"
    "• It must be simple enough for a child to picture in one sentence.\n\n"
    "SELF-CHECK before writing output:\n"
    "  Read your SCENE. Does it contain any real-world object name (cup, phone, book, etc.)? → rewrite.\n"
    "  Does it sound like a description of a desk, table, or room? → rewrite.\n"
    "  Could it describe a different set of objects? → rewrite.\n\n"
    "Step 4 — Write DRAWING INSTRUCTIONS for an illustrator.\n"
    "For each object: describe exactly what lines to draw AROUND it so it vanishes into its\n"
    "new identity. The illustrator must never draw the object itself — only the world growing out of it.\n"
    "No generic instructions. Every line must belong only to this scene and these objects.\n"
    "Reference exact positions (top-left, between X and Y, below Z, etc.).\n\n"
    "OUTPUT STRICT FORMAT (no extra text, follow exactly):\n\n"
    "SCENE: <one sentence in the language of myth — zero real object names>\n"
    "SHORT: <5 words or fewer — the scene's essence, no object names>\n\n"
    "OBJECT: <real name> | BOX: [y_min, x_min, y_max, x_max] | ROLE: <what it has become>\n"
    "...\n\n"
    "DRAWING INSTRUCTIONS:\n"
    "<for each object: what to draw around it so it disappears into its new identity>\n"
)
